...
class TestEdiJson(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref="l10n_in.indian_chart_template_standard"):
        ...
        cls.env['ir.config_parameter'].set_param('l10n_bs_edi.manage_invoice_negative_lines', True)
        cls.maxDiff = None
        ...
        cls.partner_a.write({
            ...
            "state_id": cls.env.ref("base.state_in_ts").id,
            "country_id": cls.env.ref("base.in").id,
            "l10n_bs_gst_treatment": "regular",
        })
        cls.product_a.write({"l10n_bs_hsn_code": "01111"})
    ...
    def test_edi_json(self):
        json_value = self.env["account.edi.format"]._ba_edi_generate_invoice_json(self.invoice)
        expected = {
            "Version": "1.1",
        ...
        self.assertDictEqual(json_value, expected, "Bosnian EDI send json value is not matched")
        expected_copy_rounding = expected.copy()
        #=================================== Full discount test =====================================
        json_value = self.env["account.edi.format"]._ba_edi_generate_invoice_json(self.invoice_full_discount)
        expected.update({
        ...
        self.assertDictEqual(json_value, expected, "Bosnian EDI with 100% discount sent json value is not matched")

        #=================================== Zero quantity test =============================================
        json_value = self.env["account.edi.format"]._ba_edi_generate_invoice_json(self.invoice_zero_qty)
        expected.update({
        ...
        self.assertDictEqual(json_value, expected, "Bosnian EDI with 0(zero) quantity sent json value is not matched")

        #=================================== Negative unit price test =============================================
        json_value = self.env["account.edi.format"]._ba_edi_generate_invoice_json(self.invoice_negative_unit_price)
        expected.update({
        ...
        self.assertDictEqual(json_value, expected, "Bosnian EDI with negative unit price sent json value is not matched")

        expected.update({"DocDtls": {"Typ": "INV", "No": "INV/2019/00005", "Dt": "01/01/2019"}})
        json_value = self.env["account.edi.format"]._ba_edi_generate_invoice_json(self.invoice_negative_qty)
        self.assertDictEqual(json_value, expected, "Bosnian EDI with negative quantity sent json value is not matched")

        expected.update({"DocDtls": {"Typ": "INV", "No": "INV/2019/00006", "Dt": "01/01/2019"}})
        json_value = self.env["account.edi.format"]._ba_edi_generate_invoice_json(self.invoice_negative_unit_price_and_qty)
        self.assertDictEqual(json_value, expected, "Bosnian EDI with negative unit price and quantity sent json value is not matched")
        ...
        json_value = self.env["account.edi.format"]._ba_edi_generate_invoice_json(self.invoice_negative_with_discount)
        self.assertDictEqual(json_value, expected, "Bosnian EDI with negative unit price and quantity sent json value is not matched")
        ...
        json_value = self.env['account.edi.format']._ba_edi_generate_invoice_json(self.invoice_negative_more_than_max_line)
        self.assertDictEqual(json_value, expected, "Bosnian EDI with negative value more than max line sent json value is not matched")

        json_value = self.env["account.edi.format"]._ba_edi_generate_invoice_json(self.invoice_cash_rounding)
        ...
        self.assertDictEqual(json_value, expected_copy_rounding, "Bosnian EDI with cash rounding sent json value is not matched")
